...
- Smoke run `23920869339` reached Apple notarization and captured the notary log; Apple still rejected the packaged app because the outer `openscad-studio` binary lacked hardened runtime/timestamp in the final artifact and many binaries inside the nested `OpenSCAD.app` were still considered invalid, so the workflow now uses deep sealing when re-signing the nested app and outer app bundles.
- Smoke run `23921346855` still produced the same Apple notarization rejection even after the local app bundle passed verification, which strongly suggests `tauri bundle --bundles dmg` rebuilt the app bundle that landed inside the DMG. The workflow now creates the DMG directly from the already re-signed `OpenSCAD Studio.app` bundle and verifies the mounted DMG contents before notarization so Apple sees the same bits CI verified locally.
- Smoke run `23921874918` proved the app-signing path survives into the custom DMG flow, then failed in `hdiutil create` with `Resource busy` on the arm64 runner after the staged app had already verified cleanly. The workflow now creates the DMG in `$RUNNER_TEMP`, retries the `hdiutil create` call up to three times, and clears orphaned `diskimages-helper` processes between attempts before moving the finished DMG into the bundle output directory.
